...
class LensAnalysisUI:
    def __init__(self, camera_manager, dataset_manager):
        ...
        self.apertures = []  # Initialize apertures as an empty list
        ...
        def handle_upload(e: UploadEventArguments):
            try:
                ...
                camera_settings = {
                    ...
                    'focal_length': str(tags.get('EXIF FocalLength', '')).split('/')[
                        0] if 'EXIF FocalLength' in tags else None
                }
        ...
        with dialog, ui.card().classes('p-4 min-w-[600px]'):
            ...
            try:
                ...
                settings_to_check = [
                    ...
                    'focusmode', 'focusdistance', 'focuspoint', 'focus-distance', 'manualfocusdistance', 'focallength',
                    'd162', 'd127', 'focusinfo',
    ...
    async def do_batch_capture(self, dialog, apertures):
        ...
        try:
            ...
            # Get camera config and find aperture setting
            camera_config = self.camera_manager.camera.get_config()
            aperture_settings = ['aperture', 'f-number', 'fnumber']
            setting_name = None
            
            for name in aperture_settings:
                try:
                    OK, _ = gp.gp_widget_get_child_by_name(camera_config, name)
                    if OK >= gp.GP_OK:
                        setting_name = name
                        break
                except Exception as e:
            ...
            # Proceed with batch capture
            for aperture in apertures:
                self.update_progress(f"Capturing photo with aperture {aperture}...")
                try:
                    await self.do_capture(dataset_path, aperture)
                    self.update_progress(f"Captured photo with aperture {aperture}")
                except Exception as e:
                    logging.error(f"Error capturing with aperture {aperture}: {e}")
                    ui.notify(f"Failed to capture with aperture {aperture}", type='negative')

            ui.notify('Batch capture completed', type='positive')
